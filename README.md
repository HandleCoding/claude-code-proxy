...
| `CCP_CODEX_PROXY`      | `codex.proxy`       | unset                            | Proxy URL for Codex API, OAuth login, and token refresh requests                               |
| `HTTPS_PROXY`          | —                   | unset                            | Fallback proxy URL for Codex requests                                                          |
| `HTTP_PROXY`           | —                   | unset                            | Fallback proxy URL when `HTTPS_PROXY` is unset                                                 |
| `CCP_CODEX_ORIGINATOR` | `codex.originator`  | `codex_cli_rs`                   | Override the `originator` header sent to Codex                                                 |
| `CCP_CODEX_USER_AGENT` | `codex.userAgent`   | `codex-cli-rs/<version>`         | Override the `User-Agent` header sent to Codex                                                 |
| `CCP_KIMI_USER_AGENT`  | `kimi.userAgent`    | `KimiCLI/1.37.0`                 | Override the `User-Agent` header sent to Kimi                                                  |
| `CCP_ORIGINATOR`       | —                   | unset                            | Fallback for `CCP_CODEX_ORIGINATOR`                                                            |
